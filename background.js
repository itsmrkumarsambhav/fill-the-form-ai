...
const RENDER_BACKEND_URL = "https://fill-the-form-ai.onrender.com"; 

async function callGeminiRawText(userKeys, parts) {
    const url = `${RENDER_BACKEND_URL}/api/gemini/raw`;
    const creds = await globalThis.firebaseService.getUserCredentials();
    const payload = { parts: parts, userKeys: userKeys || [], idToken: creds?.idToken || null };
